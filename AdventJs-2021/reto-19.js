...
export default function learn(time, courses) {
    let combinations = [[], [courses[0]]]
    const firstElement = courses.shift()

    courses.forEach(course => {

        const newCombinations = combinations.map(combination => {
            const combinationsCopy = [...combination]
            combinationsCopy.length < 2 && combinationsCopy.push(course)
            return combinationsCopy
        })
        combinations = [...combinations, ...newCombinations]
    })

    const posibleCombinations = combinations.filter(combination => {
        if (combination.length === 2 && combination[0] + combination[1] <= time) return combination
    })
    if (posibleCombinations.length === 0) return null

    const totalCombinations = posibleCombinations.map(item => item[0] + item[1])
    const maxCourses = Math.max(...totalCombinations)
    const maxCoursesIndex = totalCombinations.indexOf(maxCourses)
    courses.unshift(firstElement)

    let firstNumber = courses.indexOf(posibleCombinations[maxCoursesIndex][0])
    courses.splice(firstNumber, 1, null)
    let secondNumber = courses.indexOf(posibleCombinations[maxCoursesIndex][1])
