...
export default function countDecorations(bigTree) {
    ...
    const countValues = (direction) => {
        if (direction.value) totalValues += direction.value
        if (direction.left) countValues(direction.left)
        if (direction.right) countValues(direction.right)
        return
    }
