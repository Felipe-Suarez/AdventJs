...
export default function canReconfigure(from, to) {
    ...
    const res = {}
    const a1 = Array.from(from)
    const a2 = Array.from(to)
    const cacheVales = []

    a1.forEach((v, i) => {
        if (cacheVales.includes(a2[i]) && res[v] !== a2[i]) return false
        if (res[v] && res[v] !== a2[i]) return false
        if (res[v] && res[v] === a2[i]) res[v + i] = a2[i]
        cacheVales.push(a2[i])
        res[v] = a2[i]
    })
    return Object.keys(res).length === from.length
}