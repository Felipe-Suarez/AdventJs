...
export default function checkSledJump(heights) {
    ...
    const parabola = heights.reduce((acc, cur, index) => {
        if (acc > cur && index === 1) return
        if (acc < cur && goingDown === false) return cur
        if (acc > cur) {
            goingDown = true
            return cur
        }
        if (acc < cur && goingDown === true) return
    })

    if (typeof (parabola) === 'number' && goingDown) return true
    return false
}